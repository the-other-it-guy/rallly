...
<ParamField path="ANONYMOUS_AUTH_ENABLED" default="true">
  Set to "false" to disable anonymous/guest user authentication. When enabled,
  users can create and participate in polls without creating an account. 
  Anonymous users are assigned temporary email addresses in the format 
  `temp-{randomid}@rallly.co` and appear as "Guest" in the application.
  These users can later convert their anonymous account to a full registered account.
  
  <Note>When you see users with `temp-*@rallly.co` email addresses in your database, 
  these are anonymous users who have created polls without registering. This is normal 
  and part of the anonymous authentication feature.</Note>
</ParamField>
